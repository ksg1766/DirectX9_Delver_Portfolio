...
CNpc_Alchemist::CNpc_Alchemist(const CNpc_Alchemist& rhs)
	: Engine::CNpc(rhs)
{
}

CNpc_Alchemist::~CNpc_Alchemist()
{
}

HRESULT CNpc_Alchemist::Ready_Object()
{
	m_eObjectTag = OBJECTTAG::NPC;
	m_eNPCTag = NPCTAG::ALCHEMIST;
...
_int CNpc_Alchemist::Update_Object(const _float& fTimeDelta)
{
...
void CNpc_Alchemist::LateUpdate_Object()
{
...
void CNpc_Alchemist::Render_Object()
{
...
void CNpc_Alchemist::OnCollisionEnter(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
...
void CNpc_Alchemist::OnCollisionStay(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
}

void CNpc_Alchemist::OnCollisionExit(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
}

HRESULT CNpc_Alchemist::Add_Component()
{
...
#pragma region 텍스쳐
	pComponent = m_pTexture[(_uint)STATE::IDLE] = dynamic_cast<CTexture*>(Engine::PrototypeManager()->Clone_Proto(L"Proto_Texture_NpcAlchemist"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CNpc_Alchemist* CNpc_Alchemist::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CNpc_Alchemist* pInstance = new CNpc_Alchemist(pGraphicDev);

	if (FAILED(pInstance->Ready_Object()))
	{
		Safe_Release<CNpc_Alchemist*>(pInstance);
