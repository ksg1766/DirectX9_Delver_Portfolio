...
_int COrb::Update_Object(const _float& fTimeDelta)
{
	...
	CItem* ItemType = static_cast<CItem*>(rPlayer.Get_CurrentEquipRight());
	ITEMTYPEID	ItemID = {};
...
HRESULT COrb::Add_Component(void)
{
	...
	pComponent = m_pTexture = static_cast<CTexture*>(Engine::PrototypeManager()->Clone_Proto(L"Proto_Texture_itemUI"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
