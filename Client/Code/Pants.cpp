...
CPants::CPants(const CPants& rhs)
	: Engine::CItem(rhs)
{
}
...
HRESULT CPants::Ready_Object(_bool _Item)
{
	...
	m_ItemID.eItemID = ITEMID::EQUIP_OLDTROUSERS;
	m_ItemID.iCount = 1;
...
_int CPants::Update_Object(const _float& fTimeDelta)
{
...
void CPants::LateUpdate_Object(void)
{
...
void CPants::Render_Object(void)
{
...
HRESULT CPants::Add_Component(void)
{
	...
	pComponent = m_pTexture = dynamic_cast<CTexture*>(Engine::PrototypeManager()->Clone_Proto(L"Proto_Texture_Pants"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
void CPants::OnCollisionEnter(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
...
void CPants::OnCollisionStay(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
...
void CPants::OnCollisionExit(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
...
CPants* CPants::Create(LPDIRECT3DDEVICE9 pGraphicDev, _bool _Item)
{
