...
void CUISpeech_Wizard::Render_Object()
{
	...
	if (eTargetNpc == nullptr)
		return;
	if (static_cast<CNpc_Wizard*>(eTargetNpc)->IsTalk())
	{

		if (static_cast<CNpc_Wizard*>(eTargetNpc)->Get_SpeechCount() == 0)
		{
			static_cast<CFont*>(m_pFont[0])->Set_pFont(m_pFontconfig[0]);
		...
		else if (1 == static_cast<CNpc_Wizard*>(eTargetNpc)->Get_SpeechCount())
		{
			static_cast<CFont*>(m_pFont[2])->Set_pFont(m_pFontconfig[2]);
			m_pFont[2]->DrawText(L"꼭, 몬스터들을 무찌르고 오브를 찾아 이 마을을 구해주길 바랄게.");
		}
		else if (2 == static_cast<CNpc_Wizard*>(eTargetNpc)->Get_SpeechCount())
		{
			static_cast<CFont*>(m_pFont[2])->Set_pFont(m_pFontconfig[2]);
