...
void CUIUseShop_Trander::Render_Object()
{
	...
	if (static_cast<CNpc_Trader*>(eTargetNpc)->IsTalk() && pItem == nullptr)
	{

		if (static_cast<CNpc_Trader*>(eTargetNpc)->Get_SpeechCount() == 0)
		{
			static_cast<CFont*>(m_pFont[0])->Set_pFont(m_pFontconfig[0]);
		...
		else if (1 == static_cast<CNpc_Trader*>(eTargetNpc)->Get_SpeechCount())
		{
			static_cast<CFont*>(m_pFont[0])->Set_pFont(m_pFontconfig[0]);
		...
		else if (2 == static_cast<CNpc_Trader*>(eTargetNpc)->Get_SpeechCount())
		{
			static_cast<CFont*>(m_pFont[0])->Set_pFont(m_pFontconfig[2]);
			m_pFont[2]->DrawText(L"부디 오브를 찾아 이 마을을 꼭 구해주세요");
		}
	}
	else if (static_cast<CNpc_Trader*>(eTargetNpc)->IsTalk() && pItem != nullptr)
	{
		if (static_cast<CNpc_Trader*>(eTargetNpc)->Get_SpeechCount() == 0)
		{
			static_cast<CFont*>(m_pFont[2])->Set_pFont(m_pFontconfig[2]);
