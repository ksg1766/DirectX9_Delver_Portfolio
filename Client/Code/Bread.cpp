...
CBread::CBread(const CBread& rhs)
	: Engine::CItem(rhs)
{
}
...
HRESULT CBread::Ready_Object(_bool _Item)
{
	...
	m_ItemID.eItemID = ITEMID::EAT_SLICEDBREAD;
	m_ItemID.iCount = 1;
...
void CBread::LateUpdate_Object(void)
{
...
void CBread::Render_Object(void)
{
...
HRESULT CBread::Add_Component(void)
{
	...
	pComponent = m_pTexture = dynamic_cast<CTexture*>(Engine::PrototypeManager()->Clone_Proto(L"Proto_Texture_Bread"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
void CBread::OnCollisionEnter(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
...
void CBread::OnCollisionStay(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
...
void CBread::OnCollisionExit(CCollider* _pOther)
{
	if (SceneManager()->Get_GameStop()) { return; }
...
CBread* CBread::Create(LPDIRECT3DDEVICE9 pGraphicDev, _bool _Item)
{
