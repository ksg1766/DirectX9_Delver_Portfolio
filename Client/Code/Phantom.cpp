...
void CPhantom::OnCollisionEnter(CCollider* _pOther)
{
	...
	{
		_uint iPuzzleCount = static_cast<CPlayer*>(_pOther->Get_Host())->Get_PuzzleResult();
		if (iPuzzleCount == 6)
			m_bQuestClear = true;
...
#pragma region 텍스쳐
	pComponent = m_pTexture[(_uint)STATE::IDLE] = static_cast<CTexture*>(Engine::PrototypeManager()->Clone_Proto(L"Proto_Texture_NpcPhantom"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
void CPhantom::Create_Puzzle()
{
    ...
    static_cast<CTempUI*>(pGameObject)->Set_UIObjID(UIOBJECTTTAG::UIID_PICTURE, 0);
