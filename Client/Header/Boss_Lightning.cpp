...
HRESULT CBoss_Lightning::Ready_Object(void)
{
	...
	m_bHit = false;
	m_fScale = 1.;
	m_pBasicStat->Get_Stat()->fAttack = 1.2f;
	return S_OK;
...
void CBoss_Lightning::OnCollisionStay(CCollider* _pOther)
{
	...
	if (OBJECTTAG::PLAYER == _pOther->Get_Host()->Get_ObjectTag())
	{
		...
		_vec3 m_fDis = (static_cast<CPlayer*>(_pOther->Get_Host())->m_pTransform->m_vInfo[INFO_LOOK] * 1.1f);
		(static_cast<CPlayer*>(_pOther->Get_Host())->Get_RigidBody()->Add_Force(_vec3(-m_fDis.x, 1.1f, -m_fDis.z)));
...
HRESULT CBoss_Lightning::Add_Component(void)
{
	...
	pComponent = m_pTexture = static_cast<CTexture*>(Engine::PrototypeManager()->Clone_Proto(L"Proto_Texture_BossLightning"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
